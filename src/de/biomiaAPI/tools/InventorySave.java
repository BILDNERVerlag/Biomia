...
class InventorySave {
	...
	public static void saveInventory(Player p, String serverGroup) {
		...
		if (MySQL.executeQuery(
				"SELECT * from InventorySaves where uuid = '" + uuid + "' AND servergroup = '" + serverGroup + "'",
				"inventory") != null) {

			MySQL.executeUpdate(
					"UPDATE `InventorySaves` SET `uuid`= '" + uuid + "',`inventory`='" + inv + "',`servergroup`='"
							+ serverGroup + "' WHERE uuid = '" + uuid + "' AND servergroup = '" + serverGroup + "'");
		} else {
			MySQL.executeUpdate("INSERT INTO `InventorySaves` (`uuid`, `inventory`, `servergroup`) VALUES ('" + uuid
					+ "','" + inv + "','" + serverGroup + "')");
		}
