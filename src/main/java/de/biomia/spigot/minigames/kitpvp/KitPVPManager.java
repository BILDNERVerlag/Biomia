...
public class KitPVPManager {
    ...
    public static void openSelectorInventory(BiomiaPlayer bp) {
        ...
        for (int i = 0; i < maxKits; i++) {
            ...
            ItemStack is = kit != null ? Arrays.stream(kit.getInventory()).filter(itemStack -> itemStack == null || (itemStack != null && itemStack.getType() == Material.AIR)).findFirst().orElse(ItemCreator.itemCreate(Material.GLASS)) : ItemCreator.itemCreate(Material.BEDROCK);
