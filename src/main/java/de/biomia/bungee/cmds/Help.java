...
public class Help extends Command {
    ...
    public void execute(CommandSender sender, String[] arg1) {
        ...
        TextComponent biomia = new TextComponent(
                ChatColor.GOLD + "/biomia " + ChatColor.GREEN + "um Links und die IP von unserem TS zu sehen!");
        biomia.setClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, "/biomia"));

        TextComponent hub = new TextComponent(
                ChatColor.GOLD + "/hub " + ChatColor.GREEN + "um zur Lobby zurückzukehren");
        hub.setClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, "/hub"));

        TextComponent register = new TextComponent(
                ChatColor.GOLD + "/register " + ChatColor.GREEN + "um dich zu registrieren");
        register.setClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, "/register"));

        TextComponent coins = new TextComponent(
                ChatColor.GOLD + "/coins " + ChatColor.GREEN + " um deinen BC-Kontostand abzurufen");
        coins.setClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, "/coins"));

        TextComponent party = new TextComponent(
                ChatColor.GOLD + "/party " + ChatColor.GREEN + "um andere Spieler in deine Party einzuladen");
