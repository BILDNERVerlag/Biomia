package de.biomia.minigames.skywars.stats;//package de.biomia.minigames.skywars.stats;
//
...
//
//			if (MySQL.executeQuerygetint("SELECT biomiaID from SkyWarsStats WHERE biomiaID = " + stat.uuid, "biomiaID") != -1) {
//
//				MySQL.executeUpdate("UPDATE `SkyWarsStats` SET `kills`= " + stat.kills + " , `name`= '" + stat.name
//						+ "' ,`deaths`=" + stat.deaths + ",`wins`=" + stat.wins + ",`played_games`=" + stat.played_games
//						+ " WHERE uuid = '" + stat.uuid + "'");
//			} else {
//				MySQL.executeUpdate(
//						"INSERT INTO `SkyWarsStats`(`kills`, `deaths`, `wins`, `played_games`, `rank`, `uuid`, `name`) VALUES ("
...
//		if (con != null) {
//			try {
//				PreparedStatement ps = con.prepareStatement("Select * FROM `SkyWarsStats` where uuid = ?");
...
//		if (con != null) {
//			try {
//				PreparedStatement ps = con.prepareStatement("Select * FROM `SkyWarsStats` where rank = ?");
//				ps.setInt(1, rank);
//				ResultSet rs = ps.executeQuery();
//				while (rs.next()) {
...
//		if (con != null) {
//			try {
//				PreparedStatement sql = con.prepareStatement("SELECT uuid, rank FROM SkyWarsStats ORDER BY wins DESC");
//				ResultSet rs = sql.executeQuery();
//				int i = 1;
//				while (rs.next()) {
//
//					if (i != rs.getInt("rank")) {
//						MySQL.executeUpdate("UPDATE `SkyWarsStats` SET `rank`= " + i + " WHERE uuid = '"
//								+ rs.getString("uuid") + "'");
//					}
//
//					i++;
